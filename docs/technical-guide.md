...
本文是 webchat2api 0.0.11 的 canonical 技术手册，面向部署、集成、运维和排障。文档只描述当前实现，不代表 OpenAI、xAI 或 Google 官方 API 支持。
...
- 试验页支持文生文聊天、文本模型批量可用性测试、按 provider 过滤的文生图/图生图（内置 ImageComposer 组合式图片输入框，支持 inline prompt/settings、选择预设图片比例和可折叠紧凑输入栏）、图片队列和图片历史。
...
| 分组 | 模型 | 路由与说明 |
| --- | --- | --- |
| GPT 动态模型 | 聚合匿名目录和各 `provider=gpt` 账号类型的 picker categories | 只展示上游实际可用模型，不注入固定文本模型；思考档位使用 `-standard`、`-extended`、`-max` 通用后缀。 |
...
- 图像试验页内置了 `ImageComposer` 输入框组件，提供以下交互特性：
  - 粘贴图片或点击按钮上传参考图。
  - 集成 inline settings，支持实时配置和查看剩余额度、处理中的活动任务数、生成张数、图片比例、服务商和模型。
  - 图片比例下拉框支持选择预设比例：未指定（`__unset__` 语义，传入空值）、1:1（正方形）、16:9（横版）、4:3（横版）、3:4（竖版）与 9:16（竖版）。
  - 提供可展开/收起的紧凑输入栏：折叠状态下呈窄条，显示当前生成模式/编辑模式标识以及提示词（prompt）预览，便于查看生成结果或历史图片。
