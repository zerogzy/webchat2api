...
本文是 webchat2api 0.0.9 的 canonical 技术手册，面向部署、集成、运维和排障。文档只描述当前实现，不代表 OpenAI、xAI 或 Google 官方 API 支持。
...
- 文本 API 支持 GPT、Grok 与 Gemini 三类 provider，`/v1/chat/completions` 按请求 `model` 分发。
...
`x-api-key: <密钥>` 适用于 `/v1/models`、`/v1/chat/completions`、`/v1/responses`、`/v1/messages`、`/openai/v1/models`、`/openai/v1/chat/completions` 和 `/claude/v1/messages`。图片接口、Gemini native 接口与管理接口仍使用 Bearer Token。
...
- Grok 账号导入时设置 `provider=grok`；Gemini 账号导入时设置 `provider=gemini`，凭据需包含 `__Secure-1PSID`，可包含 `__Secure-1PSIDTS`；GPT 账号使用 `provider=gpt` 或默认值。
