...
| FlareSolverr clearance 刷新 | ⚠️ | 配置 `flaresolverr_url` 后，直接 app-chat 请求遇到 403 时会尝试刷新 Cloudflare Cookie 并重试；这是 best-effort clearance 支持，不保证绕过所有 Cloudflare、WAF 或账号限制。403 如果仍失败会返回给调用方，不会自动回退 Browser Bridge。 |
