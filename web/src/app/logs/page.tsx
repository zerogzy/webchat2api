...
function LogsContent() {
  ...
  const currentPageSelected =
    ...
    currentRows.every((item) => selectedSet.has(item.id));
  const allSelected =
  ...
  return (
    <section className="flex flex-col gap-5">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:flex lg:items-center">
          ...
          <Button
            ...
            className="h-10 rounded-xl border-stone-200 bg-white/90 px-4 text-stone-700 shadow-sm hover:bg-white"
          >
            清除筛选条件
          </Button>
          <Button
            ...
            disabled={isLoading}
            className="h-10 rounded-xl bg-stone-950 px-4 text-white shadow-[0_18px_40px_-26px_rgba(68,64,60,0.9)] hover:bg-stone-800"
          >
            {isLoading ? (
      ...
      <div className="overflow-hidden rounded-2xl border border-stone-200/80 bg-white/72">
          ...
          <Table className="min-w-[900px]">
            <TableHeader>
              <TableRow>
                ...
                {isCallLog ? (
                  <TableHead className="w-36">图片</TableHead>
                ) : null}
            ...
            <TableBody>
              {currentRows.map((item) => {
                ...
                return (
                  <TableRow
                    key={item.id}
                    className="text-stone-600 hover:bg-stone-50/80"
                  >
                    <TableCell>
                      <Checkbox
                        checked={selectedSet.has(item.id)}
                        onCheckedChange={(checked) =>
                          toggleIds([item.id], Boolean(checked))
                        }
                      />
                    </TableCell>
                    ...
                    {isCallLog ? (
                      <TableCell>{getDetailText(item, "key_name")}</TableCell>
                    ) : null}
                    {isCallLog ? (
                          ...
                          <div className="flex items-center gap-1.5">
                            {urls.slice(0, 3).map((url, imageIndex) => (
                              <button
                                key={`${url}-${imageIndex}`}
                                type="button"
                                className="relative size-9 overflow-hidden rounded-lg border border-stone-200 bg-stone-100 shadow-sm transition hover:scale-105"
                                onClick={() => openLogImage(item, imageIndex)}
                                title="预览图片"
                              >
                                <ImageThumbnail
                                  src={url}
                                  thumbnailSrc={getImageThumbnailUrl(url)}
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          className="h-8 rounded-lg px-3 text-stone-600 hover:bg-stone-100 hover:text-stone-900"
        ...
        <div className="flex flex-col gap-3 border-t border-stone-200/70 bg-stone-50/45 px-4 py-3 text-sm text-stone-500 sm:flex-row sm:items-center sm:justify-end">
          <span>
            第 {safePage} / {pageCount} 页，共 {items.length} 条
          </span>
          <Button
            variant="outline"
            size="icon"
        ...
        {!isLoading && items.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center text-sm text-stone-500">
            ...
            <div className="space-y-1">
              <p className="font-medium text-stone-700">没有找到日志</p>
              <p>调整日期或日志类型后重试。</p>
            </div>
          </div>
        ) : null}
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="flex h-[min(88vh,860px)] w-[min(92vw,920px)] flex-col overflow-hidden rounded-2xl border-stone-200 bg-white p-0 shadow-sm">
          <DialogHeader className="shrink-0 border-b border-stone-100 px-6 py-5">
            <DialogTitle>日志详情</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <div className="space-y-4">
              <div className="grid gap-3 border-y border-stone-200/80 py-4 text-sm text-stone-600 md:grid-cols-2">
                {Object.entries(detailLog?.detail || {})
                  .filter(
                    ([key, value]) =>
                      key !== "urls" && typeof value !== "object",
                  )
                  .map(([key, value]) => (
                    <div
                      key={key}
              ...
              <pre className="max-h-[72vh] overflow-auto rounded-xl border border-stone-200/80 bg-stone-50/80 p-4 text-xs leading-6 text-stone-700">
      ...
      <Dialog
        ...
        onOpenChange={(open) => (!open ? setDeletingItems([]) : null)}
      >
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>
              {deletingItems.length === 1 ? "删除日志" : "删除所选日志"}
            </DialogTitle>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isDeleting || deletingItems.length === 0}
            >
              {isDeleting ? (
                <LoaderCircle className="size-4 animate-spin" />
              ) : null}
