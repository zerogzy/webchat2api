...
type ImageComposerProps = {
  ...
  imageProviderOptions: Array<{ value: string; label: string; disabled?: boolean; reason?: string }>;
...
export function ImageComposer({
  ...
  const hasSelectableImageModel = imageModels.length > 0 && imageModels.includes(imageModel);
  const imageModelPlaceholder = imageModels.length === 0 ? "暂无可用模型" : "请选择模型";
  ...
  return (
    <div className="shrink-0 flex justify-center px-1 sm:px-0">
      <div style={{ width: "min(980px, 100%)" }}>
        ...
        {imageModeUnavailableMessage ? (
          <p className="mb-2 rounded-2xl border border-amber-200/80 bg-amber-50/90 px-3 py-2 text-xs leading-5 text-amber-700 shadow-sm sm:mb-3">
            {imageModeUnavailableMessage}
          </p>
        ) : null}

        {referenceImages.length > 0 ? (
          <div className="mb-2 rounded-[22px] border border-stone-200/80 bg-white/65 p-2 shadow-sm backdrop-blur-sm sm:mb-3 sm:p-3">
            <div className="mb-2 flex items-center justify-between gap-2 px-1">
              <span className="text-[11px] font-semibold tracking-[0.14em] text-stone-500 uppercase">参考图</span>
              <span className="rounded-full bg-stone-100 px-2 py-0.5 text-[11px] font-medium text-stone-500">{referenceImages.length} 张</span>
            </div>
        ...
        <div className="overflow-hidden rounded-[28px] border border-white/80 bg-white/82 shadow-[var(--shadow-lift)] backdrop-blur-xl sm:rounded-[34px]">
          <div
          ...
          >
            ...
            <Textarea
              ...
              className="min-h-[112px] resize-none rounded-[28px] border-0 bg-transparent px-4 pt-4 pb-3 text-[15px] leading-6 text-stone-900 shadow-none placeholder:text-stone-400 focus-visible:ring-0 sm:min-h-[170px] sm:rounded-[34px] sm:px-6 sm:pt-6 sm:pb-28 sm:leading-7"
            />

            <div className="border-t border-stone-100 bg-[linear-gradient(180deg,rgba(255,255,255,0.82),rgba(250,247,239,0.96))] px-3 pt-3 pb-3 sm:absolute sm:inset-x-4 sm:bottom-4 sm:rounded-[24px] sm:border sm:border-white/80 sm:px-4 sm:shadow-[0_18px_54px_-42px_rgba(68,64,60,0.75)]" onClick={(event) => event.stopPropagation()}>
              <div className="mb-2 hidden items-center justify-between gap-2 text-[11px] font-semibold tracking-[0.14em] text-stone-500 uppercase sm:flex">
                <span>生成设置</span>
                <span>{referenceImages.length > 0 ? "编辑模式" : "生成模式"}</span>
              </div>
              <div className="flex items-end justify-between gap-2 sm:gap-3">
                <div className="hide-scrollbar flex min-w-0 flex-1 flex-nowrap items-center gap-1.5 overflow-x-auto pb-0.5 sm:flex-wrap sm:gap-3 sm:overflow-visible sm:pb-0">
                  ...
                  <div className="flex h-9 min-w-[150px] shrink-0 items-center gap-1.5 rounded-full border border-stone-200 bg-white px-2 py-0.5 sm:h-auto sm:min-w-[178px] sm:gap-2 sm:px-3 sm:py-1">
                    <span className="hidden text-[11px] font-medium text-stone-700 sm:inline sm:text-sm">服务</span>
                    <Select value={imageProvider} onValueChange={onImageProviderChange}>
                      ...
                      <SelectContent>
                        {imageProviderOptions.map((option) => (
                          <SelectItem key={option.value} value={option.value} disabled={option.disabled}>
                            <span>{option.label}</span>
                            {option.reason ? <span className="ml-2 text-xs text-stone-400">{option.reason}</span> : null}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="flex h-9 min-w-[168px] shrink-0 items-center gap-1.5 rounded-full border border-stone-200 bg-white px-2 py-0.5 sm:h-auto sm:min-w-[220px] sm:gap-2 sm:px-3 sm:py-1">
                    ...
                    {hasSelectableImageModel ? (
                      ...
                      <button
                        ...
                        aria-label={imageModels.length === 0 ? "当前服务暂无可用图像模型" : "请选择图像模型"}
                      >
                ...
                <button
                  ...
                  disabled={!prompt.trim() || Boolean(imageModeUnavailableMessage)}
