...
AUTH_FAILURE_MARKERS = ("auth", "unauthorized", "forbidden", "invalid", "expired", "credential", "bearer")
SECRET_KEYS = ("bearer_token", "access_token", "accessToken", "token")
...
def normalize_access_token(item: dict[str, Any]) -> str:
    return _clean(item.get("account_id") or item.get("user_id") or item.get("bearer_token") or item.get("access_token") or item.get("token"))


def normalize_account(account: dict[str, Any]) -> dict[str, Any]:
    bearer = _clean(account.get("bearer_token") or account.get("access_token") or account.get("token"))
    identity = _clean(account.get("account_id") or account.get("user_id"))
    if not identity and bearer:
        identity = "codebuddy:" + hashlib.sha256(bearer.encode()).hexdigest()[:16]
...
def account_row_id(account: dict[str, Any]) -> str:
    ...
    return hashlib.sha256(f"codebuddy\0{identity}".encode()).hexdigest() if identity else ""
...
def sanitize_account(item: dict[str, Any]) -> dict[str, Any]:
    ...
    account["has_bearer_token"] = bool(_clean(item.get("bearer_token")))
...
def delete_token_matches_account(token: str, account: dict[str, Any]) -> bool:
    text = _clean(token)
    return text in {normalize_access_token(account), _clean(account.get("bearer_token")), _clean(account.get("user_id"))}
...
def refresh_error_message(exc: Exception) -> str:
    return str(exc) or "CodeBuddy credential validation failed"


def export_filename() -> str:
    return "webchat2api_codebuddy.txt"


def build_export_item(account: dict[str, Any]) -> dict[str, str] | None:
    token = _clean(account.get("bearer_token"))
    if not token:
        return None
    return {"provider": "codebuddy", "bearer_token": token, "user_id": _clean(account.get("user_id"))}
...
def account_has_capability(account: dict[str, Any], spec: Any) -> bool:
    return bool(_clean(account.get("bearer_token")))
