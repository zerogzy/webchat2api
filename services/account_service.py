...
class AccountService:
    ...
    def _normalize_account(self, item: dict) -> dict | None:
        ...
        if normalized["provider"] in {GROK_PROVIDER, GEMINI_PROVIDER, CATPAW_PROVIDER, JOYCODE_PROVIDER, CODEBUDDY_PROVIDER, QODER_PROVIDER}:
    ...
    def fetch_remote_info(self, access_token: str, event: str = "fetch_remote_info", provider: str | None = None) -> dict[str, Any] | None:
        ...
        if account_provider == CODEBUDDY_PROVIDER:
            return dict(account) if account else None
        if account_provider == QODER_PROVIDER:
            return dict(account) if account else None
    ...
    def _resolve_identifier_tokens_locked(self, identifiers: list[dict[str, str]] | None = None, provider_filter: str | None = None) -> list[str]:
        ...
        candidate_providers = [provider_filter] if provider_filter else [GEMINI_PROVIDER, GROK_PROVIDER, CATPAW_PROVIDER, CODEBUDDY_PROVIDER, QODER_PROVIDER]
    ...
    def build_export_items(
        ...
        if provider_filter not in {None, GPT_PROVIDER, GROK_PROVIDER, GEMINI_PROVIDER, CATPAW_PROVIDER, JOYCODE_PROVIDER, CODEBUDDY_PROVIDER, QODER_PROVIDER}:
            return []
        with self._lock:
